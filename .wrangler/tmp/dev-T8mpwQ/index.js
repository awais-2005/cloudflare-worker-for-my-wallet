...
var secret = /* @__PURE__ */ __name((env) => encoder.encode(env.JWT_SECRET), "secret");
async function signJwt(payload, env) {
  const header = { alg: "HS256", typ: "JWT" };
  const base64 = /* @__PURE__ */ __name((obj) => btoa(JSON.stringify(obj)).replace(/=+$/, "").replace(/\+/g, "-").replace(/\//g, "_"), "base64");
...
__name(otpGenerator, "otpGenerator");
...
async function login(c) {
  ...
  return c.json({ id: user.id, token });
}
__name(login, "login");
...
// src/utils/verifyPayload.js
async function verifyPayloadWithDb(payload, db) {
...
__name(verifyPayloadWithDb, "verifyPayloadWithDb");
...
// src/routes/auth.js
function handleAuthRoutes(app2) {
  app2.post("/auth/register", validateRegister, register);
  app2.post("/auth/login", validateLogin, login);
  app2.post("/auth/otp", authGuard, sendOTP);
}
__name(handleAuthRoutes, "handleAuthRoutes");

// src/models/user.js
async function setUserAvatar(db, id, avatarUrl) {
  return await db.prepare("UPDATE users SET avatar = ? WHERE id = ?").bind(avatarUrl, id).run();
}
__name(setUserAvatar, "setUserAvatar");

// src/controllers/user.controller.js
async function setAvatar(c) {
  const db = c.env.DB;
  const user = c.get("user");
...
__name(updateCurrency, "updateCurrency");
...
// src/routes/user.js
function handleUserRoutes(app2) {
  app2.get("/users", listUsers);
  app2.get("/user/:id", getUser);
  app2.get("/user/info", authGuard, getUserInfo);
  app2.put("/user/update", authGuard, updateUser);
  app2.put("/user/avatar", authGuard, setAvatar);
  app2.put("/user/setpassword/:id", updatePassword);
  app2.put("/user/setcurrency/:id", updateCurrency);
  app2.delete("/user/delete/:id", deleteById);
}
...
function handleHealthRoutes(app2) {
  app2.get("/health", healthCheck);
}
__name(handleHealthRoutes, "handleHealthRoutes");
...
async function listTransactions(c) {
...
async function createTransaction(c) {
  ...
  const result = await db.prepare("INSERT INTO transactions (user_id, amount, type, description) VALUES (?, ?, ?, ?)").bind(user_id, amount, type, description).run();
  console.log(result);
...
__name(deleteTxById, "deleteTxById");
async function updateTransaction(c) {
  let id;
  try {
    id = c.req.param("id");
  } catch (err) {
...
async function saveTransaction(c) {
  ...
  const ids = results.map((result) => Number(result?.meta?.last_row_id)).filter((id) => Number.isInteger(id) && id > 0);
  return c.json({ message: "Transactions inserted successfully.", ids });
}
__name(saveTransaction, "saveTransaction");

// src/routes/transaction.js
function handleTransactionRoutes(app2) {
  app2.get("/transactions", authGuard, listTransactions);
  app2.post("/transaction/new", authGuard, createTransaction);
  app2.post("/transaction/save", authGuard, saveTransaction);
  app2.put("/transaction/update/:id", authGuard, updateTransaction);
  app2.delete("/transaction/delete/:id", authGuard, deleteTxById);
}
__name(handleTransactionRoutes, "handleTransactionRoutes");

// src/controllers/admin.controller.js
async function resetEverything(c) {
  const db = c.env.DB;
  try {
    await db.batch([
...
__name(resetEverything, "resetEverything");

// src/routes/admin.js
function handleAdminRoutes(app2) {
  app2.delete("/admin/reset", resetEverything);
}
...
async function notFoundHandler2(c) {
...
async function authorize(c, next) {
...
var cors = /* @__PURE__ */ __name((c, next) => {
...
// .wrangler/tmp/bundle-XXOymx/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XXOymx/middleware-loader.entry.ts
